...
export function EmailModal({ isOpen, onClose, defaultEmail = "" }: EmailModalProps) {
  ...
  const handleSubmit = (e: React.FormEvent) => {
    e.preventDefault()
    console.log("[v0] Email form submitted:", formData)
    // TODO: Wire to API
    onClose()
  }
  ...
  return (
    <div className="fixed inset-0 bg-black/80 z-50 flex items-center justify-center p-4">
      <div className="bg-black border border-yellow-500/30 rounded-2xl p-6 max-w-2xl w-full max-h-[90vh] overflow-y-auto">
        ...
        <form onSubmit={handleSubmit} className="space-y-4">
          <div>
            <label className="text-yellow-400/70 text-sm mb-2 block">To *</label>
            <Input
              required
              type="email"
              value={formData.to}
          ...
          <div>
            <label className="text-yellow-400/70 text-sm mb-2 block">Subject *</label>
            <Input
              required
